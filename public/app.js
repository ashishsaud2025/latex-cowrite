...
async function pollCompileStatus(projectName, jobId) {
  try {
    ...
    if (data.status === 'success') {
      logContent.textContent = data.log || '(empty)';
      if (data.cached) {
...
async function renderPdf(arrayBuffer) {
  ...
  for (let pageNumber = 1; pageNumber <= pdfDocument.numPages; pageNumber += 1) {
    ...
    const scale = Math.min(1.6, Math.max(1, (pdfContainer.clientWidth - 40) / page.getViewport({ scale: 1 }).width));
    ...
    pdfContainer.appendChild(pageView);
...
async function navigateToPdfDestination(destination) {
  ...
  const target = pdfContainer.querySelector(`[data-page-number="${pageIndex + 1}"]`);
...
async function jumpToSourceText(text) {
  if (!currentProject || !text) return 'not-found';
  if (currentFile?.editable) {
  ...
  const matches = [];
  for (const node of flattenEditableFiles(projectTree)) {
    if (currentFile?.path === node.path) continue;
    const res = await fetch(
      `/api/projects/${encodeURIComponent(currentProject)}/file?path=${encodeURIComponent(node.path)}`
    );
    if (!res.ok) continue;
  ...
  if (currentFile?.path !== matches[0].node.path) return 'not-found';
  placeSourceCursor(cm.getValue(), sourceTextPosition(cm.getValue(), text));
  return 'matched';
}

function findPdfText(text) {
  const query = normalizeSearchText(text);
  if (!query) return;
  for (const textLayer of pdfContainer.querySelectorAll('.textLayer')) {
...
cm.on('cursorActivity', () => {
  ...
  if (selection.length >= 2 && activePreviewTab === 'pdf') {
